...
int input_update()
{
    ...
    while (SDL_PollEvent(&e))
    {
        ...
        {
            int ctrl_pressed = ((e.key.keysym.mod & KMOD_CTRL) != 0 || (e.key.keysym.mod & KMOD_META) != 0);
...
Point convertMouseCoordinates(int mouseX, int mouseY)
{
    ...
    Point convertedPoint = {mouseX, mouseY};
...
void updateButtonState()
{
    ...
    buttonState.current[BUTTON_UP] = SDL_GetKeyState(NULL)[SDLK_UP];

    buttonState.previous[BUTTON_DOWN] = buttonState.current[BUTTON_DOWN];
    buttonState.current[BUTTON_DOWN] = SDL_GetKeyState(NULL)[SDLK_DOWN];

    buttonState.previous[BUTTON_LEFT] = buttonState.current[BUTTON_LEFT];
    buttonState.current[BUTTON_LEFT] = SDL_GetKeyState(NULL)[SDLK_LEFT];

    buttonState.previous[BUTTON_RIGHT] = buttonState.current[BUTTON_RIGHT];
    buttonState.current[BUTTON_RIGHT] = SDL_GetKeyState(NULL)[SDLK_RIGHT];

    buttonState.previous[BUTTON_START] = buttonState.current[BUTTON_START];
    buttonState.current[BUTTON_START] = SDL_GetKeyState(NULL)[SDLK_RETURN];

    buttonState.previous[BUTTON_SELECT] = buttonState.current[BUTTON_SELECT];
    buttonState.current[BUTTON_SELECT] = SDL_GetKeyState(NULL)[SDLK_RSHIFT];

    buttonState.previous[BUTTON_A] = buttonState.current[BUTTON_A];
    buttonState.current[BUTTON_A] = SDL_GetKeyState(NULL)[SDLK_z];

    buttonState.previous[BUTTON_B] = buttonState.current[BUTTON_B];
    buttonState.current[BUTTON_B] = SDL_GetKeyState(NULL)[SDLK_x];
}
